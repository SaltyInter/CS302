//CS 302 Assignment 10 By Jorge Fonseca
...
/*
 * Name: Matthew Shiroma, NSHE ID: 2001814396, assignment #10
 * Description:  Sith fleet with hashing
 * Inputs: File with names of ships
 * Outputs: Output of list in order with hashing
 */
...
//changes for ast10
class Hashing{
    int items;      //number of items into the list
    //pointer to a vector containing buckets
    vector<Xyston_class_StarDestroyer> *table;

public:
    //constructor
    Hashing(int num){items = num; table = new vector<Xyston_class_StarDestroyer>[items];};

    //functions
    int hashFunction(Xyston_class_StarDestroyer ship){
        //temp vars
        string tempStr = ship.getName();
        int tempInt = 0;

        //hash function to get the first i < (num) letters then
        //convert to ascii value to place at hash table
        for(int i = 0; i < 2; i++){
            //move the number two decimal places
            tempInt = tempInt * 100;
            //get the ascii value and - 65 since thats where 'A' starts
            tempInt += (int(tempStr[i]) - 65);
        }
        return tempInt;
    }
    void insertItem(Xyston_class_StarDestroyer ship){
        #ifdef COUNTERS
                hashCounter++;
        #endif
        //get the index to place it into the table
        int tableIndex = hashFunction(ship);
        //cout << "TABLE INDEX: " << tableIndex << endl; //TESTING
        table[tableIndex].push_back(ship);
    }
    void displayHashTable(){
        //print each index of the hash table
        for(int i = 0; i < items; i++){
            cout << i << ".) ";
            //print out the items at the index
            //and all the other collusions there
            for(auto x : table[i]){
                cout << " --> " << x.getName();
            }
            cout << endl;
...
int main(int argc, char **argv){
    ...
    else if(inputDataTypeStr == "hashing"){
        dataType = 5;
    }
...
//#define DEBUG
    ...
    for(int i=0; i < fleet_size; i++){
        ...
        else if(dataType == 5){     //changes for ast10
            hash.insertItem(Xyston_class_StarDestroyer(nameList));
        }
    }
